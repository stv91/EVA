...
app.controller('examMarksController', function ($scope, $http, $location, $alert) {
	...
	function changeValue(student, exam, mark, cb) {
		var data = 'student='+student;
		data += '&exam='+exam,
		data +='&mark='+mark
		$http({
	        url: 'changeexammark.html',
	        method: "POST",
	...
	$scope.validateMark = function(name) {
    	...
    	if(patt.test(value)) {
    		changeValue(name, examID, value, function(data) {
    			if(data == "ERROR"){
